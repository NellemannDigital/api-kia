...
class ProcessAdvisorActivity implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $response = Http::timeout(5)->post(
            config('services.power_automate.advisor_webhook_url'),
            $mapped
        );
    ...
    private function mapPayload(string $type, array $payload, $dealer): array
    {
        return match ($type) {
            ...
            'advisor' => [
                ...
                'selectedCar' => $payload['car']['name'] ?? null,
                'modelID' => $payload['car']['web_id'] ?? null,

                'bookingDate' => isset($payload['date'])
                    ? Carbon::parse($payload['date'])->format('d-m-Y')
                    : null,
                'bookingTimeofDay' => $payload['time'] ?? null,

                'selectedSubjects' => implode(', ', $payload['topics']) ?? '',
                'customsubject' => $payload['custom_subject'] ?? null,
