...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <section class="mx-auto w-[210mm] h-[301mm]">
            <div class="relative w-full h-full">
                @if($car->price_list->primary_image != null)
                    <img src="{{ $car->price_list->primary_image->url }}" class="bg-primary w-full h-full object-cover">
                @else
                    <div class="bg-primary w-full h-full object-cover"></div>
                @endif
        ...
        @if($car->price_list->campaign?->valid_from
            ...
            <section class="mx-auto w-[210mm] h-[301mm]">
                <div class="relative w-full h-full">
                    <img src="{{ $car->price_list->campaign->image->url }}" class="w-full h-full object-cover">
                </div>
            </section>
                        ...
                        <div>
                            ...
                            @else
                                <div class="w-full aspect-video pb-4 flex justify-center items-end">
                                    <div class="w-[65%] h-[80%] bg-primary-lowest flex items-center justify-center text-primary-mid">
        ...
        @php
            $trimEquipmentMap = $car->trims->mapWithKeys(function ($trim) {
                return [
                    $trim->id => $trim->equipment->pluck('code')->flip()
                ];
            });
        @endphp

        <section class="mx-auto max-w-4xl text-primary">

            <div class="flex justify-between items-center mb-4">
                <div class="text-xl font-bold">
                    Sammenligning af standardudstyr
                </div>
                <img src="{{ asset('images/logo.png') }}" class="block w-20">
            </div>
            ...
            <div class="space-y-6">
                @foreach($groupedEquipment as $category => $equipments)
                    ...
                    <table class="text-xs">
                        ...
                        <thead class="bg-primary font-bold text-white">
                            <tr>
                                <th class="text-left p-2 w-1/2">Standardudstyr</th>
                        ...
                        <tbody>
                            @foreach($equipments as $equipment)
                                <tr class="not-last:border-b border-primary-low even:bg-gray-50">
                                    ...
                                    @foreach($car->trims as $trim)
                                        @php
                                            $hasEquipment = isset($trimEquipmentMap[$trim->id][$equipment->code]);
                                        @endphp
